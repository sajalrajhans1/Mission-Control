...
export function NotificationsMenu() {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 origin-top-right rounded-2xl border border-zinc-200 dark:border-dark-border bg-white dark:bg-dark-base shadow-xl ring-1 ring-black/5 dark:ring-white/10 focus:outline-none z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
          {/* Header */}
          <div className="flex items-center justify-between border-b border-zinc-100 dark:border-dark-border bg-zinc-50/50 dark:bg-dark-card/50 px-4 py-3">
            <span className="text-sm font-bold text-zinc-900 dark:text-dark-text">Notifications</span>
            {unreadCount > 0 && (
              <button
          ...
          {/* Notifications List */}
          <div className="max-h-[360px] overflow-y-auto divide-y divide-zinc-100 dark:divide-dark-border/60">
            {myNotifications.map((n) => (
              <div
                key={n.id}
                onClick={() => !n.read && markAsRead(n.id)}
                className={cn(
                  "group relative flex gap-3 p-4 text-left cursor-pointer transition-colors hover:bg-zinc-50/50 dark:hover:bg-dark-hover/40",
              ...
              >
                ...
                <div className="flex-1 min-w-0 pr-6">
                  <p className={cn("text-xs font-semibold text-zinc-900 dark:text-dark-text", !n.read && "font-bold")}>
                    {n.title}
                  </p>
            ...
            {myNotifications.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-zinc-50 dark:bg-dark-card text-zinc-400">
                  <Inbox className="h-5 w-5" />
                </div>
                <p className="mt-3 text-xs font-semibold text-zinc-800 dark:text-dark-text">All caught up!</p>
          ...
          {/* Footer */}
          {myNotifications.length > 0 && (
            <div className="border-t border-zinc-100 dark:border-dark-border bg-zinc-50/30 dark:bg-dark-base/30 px-4 py-2 flex justify-end">
              <button
                onClick={clearAll}
