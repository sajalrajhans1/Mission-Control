...
export function NotificationsMenu() {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 origin-top-right rounded-2xl border border-zinc-100 bg-white shadow-xl ring-1 ring-black/5 focus:outline-none z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
          {/* Header */}
          ...
          <div className="max-h-[360px] overflow-y-auto divide-y divide-zinc-50">
            {myNotifications.map((n) => (
              <div
                ...
                className={cn(
                  "group relative flex gap-3 p-4 text-left cursor-pointer transition-colors hover:bg-zinc-50/50",
              ...
              >
                ...
                <div className="flex-1 min-w-0 pr-6">
                  <p className={cn("text-xs font-semibold text-zinc-900", !n.read && "font-bold")}>
                    {n.title}
                  </p>
                ...
                <button
                  ...
                  className="absolute right-3 top-4 hidden group-hover:flex h-6 w-6 items-center justify-center rounded-lg border bg-white text-zinc-400 hover:text-red-600 hover:border-red-100 hover:bg-red-50/40 shadow-soft transition-all"
                  title="Delete"
                >
            ...
            {myNotifications.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                ...
                <p className="mt-3 text-xs font-semibold text-zinc-800">All caught up!</p>
          ...
          {/* Footer */}
          {myNotifications.length > 0 && (
            <div className="border-t border-zinc-50 bg-zinc-50/30 px-4 py-2 flex justify-end">
              <button
                onClick={clearAll}
                className="text-[10px] font-bold uppercase tracking-wider text-red-650 hover:text-red-800 py-1 transition-colors flex items-center gap-1.5"
              >
